package com.kankanews.search.db.base;
...
public class BaseDao<T, ID extends Serializable> implements IBaseDao<T, ID> {
	 ...
	 *      java.lang.Object[])
	 */
	public ScrollableResults getListByHQLScroll(String hqlString,
			Object... values) {
		Query query = this.getSession().createQuery(hqlString);
		if (values != null) {
			for (int i = 0; i < values.length; i++) {
	 ...
	 * @Override public PageResults<T> findPageByFetchedHql(String hql, String
	 *           countHql, int pageNo, int pageSize, Object... values) {
	 *           PageResults<T> retValue = new PageResults<T>(); Query query =
	 *           this.getSession().createQuery(hql); if(values != null){ for(int
	 *           i = 0; i < values.length; i++) { query.setParameter(i,
	 *           values[i]); } } int currentPage = pageNo > 1 ? pageNo : 1;
	 *           retValue.setCurrentPage(currentPage);
	 *           retValue.setPageSize(pageSize); if (countHql == null) {
	 *           ScrollableResults results = query.scroll(); results.last();
	 *           retValue.setTotalCount(results.getRowNumber() + 1);// 设置总记录数 }
	 *           else { Long count = countByHql(countHql, values);
	 *           retValue.setTotalCount(count.intValue()); }
	 *           retValue.resetPageNo(); List<T> itemList =
	 *           query.setFirstResult((currentPage - 1) *
	 *           pageSize).setMaxResults(pageSize).list(); if (itemList == null)
